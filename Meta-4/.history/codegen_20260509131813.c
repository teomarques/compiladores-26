...
static void emit_preamble(void) {
    printf("@.fmt.d = private constant [4 x i8] c\"%%d\\0A\\00\"\n");
    printf("@.fmt.e = private constant [7 x i8] c\"%%.16e\\0A\\00\"\n");
    printf("@.fmt.s = private constant [4 x i8] c\"%%s\\0A\\00\"\n");
    printf("@.str.true = private constant [6 x i8] c\"true\\0A\\00\"\n");
    printf("@.str.false = private constant [7 x i8] c\"false\\0A\\00\"\n");
    printf("\n");
}
    ...
    case node_Print: {
        ...
        if (arg->type == node_StrLit) {
            ...
            printf("  %%t%d = getelementptr [%d x i8], [%d x i8]* %s, i32 0, i32 0\n",
                   ptr, slen, slen, sname);
            int fptr = new_temp();
            printf("  %%t%d = getelementptr [4 x i8], [4 x i8]* @.fmt.s, i32 0, i32 0\n", fptr);
            int d = new_temp();
            ...
            if (atype && strcmp(atype, "boolean") == 0) {
                ...
                printf("L%d:\n", ltr);
                int pt = new_temp();
                printf("  %%t%d = getelementptr [6 x i8], [6 x i8]* @.str.true, i32 0, i32 0\n", pt);
                int fp1 = new_temp();
                printf("  %%t%d = getelementptr [4 x i8], [4 x i8]* @.fmt.s, i32 0, i32 0\n", fp1);
                int d1 = new_temp();
                printf("  %%t%d = call i32 (i8*, ...) @printf(i8* %%t%d, i8* %%t%d)\n", d1, fp1, pt);
                ...
                printf("L%d:\n", lfa);
                int pf = new_temp();
                printf("  %%t%d = getelementptr [7 x i8], [7 x i8]* @.str.false, i32 0, i32 0\n", pf);
                int fp2 = new_temp();
                printf("  %%t%d = getelementptr [4 x i8], [4 x i8]* @.fmt.s, i32 0, i32 0\n", fp2);
                int d2 = new_temp();
                printf("  %%t%d = call i32 (i8*, ...) @printf(i8* %%t%d, i8* %%t%d)\n", d2, fp2, pf);
                ...
                int fptr = new_temp();
                printf("  %%t%d = getelementptr [7 x i8], [7 x i8]* @.fmt.e, i32 0, i32 0\n", fptr);
                int d = new_temp();
                printf("  %%t%d = call i32 (i8*, ...) @printf(i8* %%t%d, double %%t%d)\n", d, fptr, val);
            } else {
                int fptr = new_temp();
                printf("  %%t%d = getelementptr [4 x i8], [4 x i8]* @.fmt.d, i32 0, i32 0\n", fptr);
                int d = new_temp();
