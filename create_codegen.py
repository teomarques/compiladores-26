...
void emit_string_constants() {
    ...
    printf("@.str.fmt.double = private unnamed_addr constant [7 x i8] c\\"%%.16e\\\\00\\"\\n");
...
int codegen_expression(struct node *expr, ClassTable *ct, MethodEntry *mt) {
    ...
    switch (expr->category) {
        ...
        case N_Assign: {
            struct node *lhs = expr->children->node;
            struct node *rhs = expr->children->next->node;
        ...
        case N_Geq: {
            ...
            if (is_double) {
                ...
                else if (expr->category == N_Neq) cond = "one";
                else if (expr->category == N_Lt) cond = "olt";
                else if (expr->category == N_Gt) cond = "ogt";
                else if (expr->category == N_Leq) cond = "ole";
                else if (expr->category == N_Geq) cond = "oge";
                printf("  %%%d = fcmp %s double %%%d, %%%d\\n", res, cond, t1, t2);
            } else {
                if (expr->category == N_Eq) cond = "eq";
                else if (expr->category == N_Neq) cond = "ne";
                else if (expr->category == N_Lt) cond = "slt";
                else if (expr->category == N_Gt) cond = "sgt";
                else if (expr->category == N_Leq) cond = "sle";
                else if (expr->category == N_Geq) cond = "sge";
        ...
        case N_Not: {
            int t1 = codegen_expression(expr->children->node, ct, mt);
            int res = new_temp();
            printf("  %%%d = xor i1 %%%d, 1\\n", res, t1);
            return res;
        }
        ...
        case N_Call: {
            ...
            struct node_list *curr_arg = expr->children->next;
            ...
            if (strcmp(ret_type, "void") != 0) {
                res = new_temp();
                printf("  %%%d = call %s @_%s_%s(", res, ret_type, ct->name, method_name);
            } else {
                printf("  call void @_%s_%s(", ct->name, method_name);
            }
        ...
        case N_ParseArgs: {
            ...
            struct node *idx_expr = expr->children->next->node;
...
void codegen_statement(struct node *stmt, ClassTable *ct, MethodEntry *mt) {
    ...
    switch (stmt->category) {
        case N_Block: {
            struct node_list *c = stmt->children;
            while (c && c->node) {
        ...
        case N_VarDecl: {
            /* Handled in method prologue for allocas, but we must do it here to support arbitrary blocks?
             * Juc variables are method-scoped according to Java rules (but declared inside blocks). 
             * Let's allocate them right here. */
            struct node_list *c = stmt->children;
        ...
        case N_If: {
            struct node *cond = stmt->children->node;
            struct node *then_stmt = stmt->children->next->node;
            struct node *else_stmt = stmt->children->next->next ? stmt->children->next->next->node : NULL;
        ...
        case N_While: {
            struct node *cond = stmt->children->node;
            struct node *body = stmt->children->next->node;
        ...
        case N_Return: {
            struct node *ret_expr = stmt->children ? stmt->children->node : NULL;
            if (ret_expr) {
        ...
        case N_Print: {
            struct node *arg = stmt->children->node;
            if (arg->category == N_StrLit) {
                ...
                if (strcmp(llvm_type, "i32") == 0) {
                    ...
                    printf("  %%%d = getelementptr [7 x i8], [7 x i8]* @.str.fmt.double, i32 0, i32 0\\n", t_fmt);
...
void collect_local_decls(struct node *n) {
    if (!n) return;
    if (n->category == N_VarDecl) {
        struct node_list *c = n->children;
        const char *jtype = "undef";
        if (c && c->node) {
            if (c->node->category == N_Int) jtype = "int";
...
void codegen_method(struct node *method_decl, ClassTable *ct) {
    struct node_list *md = method_decl->children;
    struct node *header = md->node;
    struct node *body = md->next ? md->next->node : NULL;
    
    struct node_list *hc = header->children;
    ...
    printf("define %s @_%s_%s(", ret_llvm_type, ct->name, method_name);
    ...
    while (p && p->node) {
        ...
        struct node *ptype_node = param_decl->children->node;
    ...
    while (p && p->node) {
        struct node *param_decl = p->node;
        struct node *ptype_node = param_decl->children->node;
        struct node *pname_node = param_decl->children->next->node;
    ...
    /* Collect and allocate all local variables at the top of the function */
    if (body) {
        struct node_list *stmt = body->children;
        while (stmt && stmt->node) {
            collect_local_decls(stmt->node);
            stmt = stmt->next;
        }
    }
...
with open("/home/teomarques/Desktop/0_UC/0_2S2A/COMP/praticas/compiladores-26/Meta4/src/codegen.c", "w") as f:
    f.write(codegen_c)
